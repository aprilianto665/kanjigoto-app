...
export interface KanjiItem {
  ...
  kanji: string;     // Karakter Kanji / kata, contoh: '魚', '私'
  furigana: string;  // Cara baca Kana (hiragana/katakana), contoh: 'さかな', 'わたし'
  romaji: string;    // Cara baca Romaji, contoh: 'sakana', 'watashi'
...
- Data Kanji setiap level diekstrak lengkap hanya untuk Kanji spesifik level tersebut (tanpa duplikasi review level lama).
- Nilai `chapter` tepat sesuai kolom `か` level bersangkutan.
- Nilai Romaji tepat dan bebas salah ketik.
